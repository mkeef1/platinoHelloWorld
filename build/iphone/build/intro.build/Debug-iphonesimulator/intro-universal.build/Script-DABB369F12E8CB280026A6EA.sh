...
if [ "x$TITANIUM_CLI_XCODEBUILD" == "x" ]; then
    /usr/local/node/bin/node "/usr/local/node/bin/titanium" build --platform iphone --sdk "3.4.1.GA" --no-prompt --no-progress-bars --no-banner --no-colors --build-only --xcode
    exit $?
else
