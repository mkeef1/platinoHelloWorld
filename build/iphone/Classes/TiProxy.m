...
//Rare exceptions to indicate a bug in the _intro code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_intro = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_intro]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.intro.module"
